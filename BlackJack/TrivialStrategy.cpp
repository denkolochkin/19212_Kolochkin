...
int TrivialStrategy::details(std::vector<int> History, int CurrentCard) {
    std::cout<<"Details of trivial strategy:"<<std::endl;
    std::cout<<"New card - "<<CurrentCard<<std::endl;
    std::cout<<"Cards of trivial strategy strategy now: ";
...
int TrivialStrategy::takeCard(int CardMode, int NumberOfDecks) {
    if (CardMode == EASY) {
        return rand() % 10 + 1;
    }
    if (CardMode == DECK) {
...
int TrivialStrategy::play(int DealerCard, int Mode, int NumberOfDecks, int GameMode) {
