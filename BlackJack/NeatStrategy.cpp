...
int NeatStrategy::details(std::vector<int> History, int CurrentCard) {
    ...
    std::cout<<"Cards of neat strategy strategy now: ";
...
int NeatStrategy::takeCard(int CardMode, int NumberOfDecks) {
    if (CardMode == EASY) {
        return rand() % 10 + 1;
    }
    if (CardMode == DECK) {
...
int NeatStrategy::play(int DealerCard, int Mode, int NumberOfDecks, int GameMode) {
